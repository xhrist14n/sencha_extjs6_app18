 ...
 */
Ext.define('app18.controller.ajax.AlumnoController', {
	extend : 'Ext.app.ViewController',

	alias : 'controller.ajaxalumno',
	'nuevoAlumno' : function(button, e, eOpts) {
		var form = button.up('form');
		form.reset();
		var tab = button.up('#ajaxalumnotab');
		var grid = tab.down('#ajaxalumnogrid');
		try {
			grid.getStore().load();
	...
	'eliminarAlumno' : function(button, e, eOpts) {
		var grid = null;
		var elems = Ext.ComponentQuery.query('#ajaxalumnotab');
		var grid = null;
		for (var i in elems) {
			try {
				grid = elems[i].down('#ajaxalumnogrid');
			} catch(ex) {
			}
		...
		for (var i in elems) {
			try {
				form = elems[i].down('#ajaxalumnoform');
			} catch(ex) {
			}
